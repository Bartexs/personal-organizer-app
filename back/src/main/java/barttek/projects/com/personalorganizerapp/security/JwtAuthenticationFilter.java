        ...
        if(request.getServletPath().equals("login") || request.getServletPath().equals("register") || request.getServletPath().equals("token/refresh")) {
            ...
            if(authorizationHeader != null && authorizationHeader.startsWith("Bearer ")) {
                try {
                    ...
                    String[] appUserRole = decodedJWT.getClaim("appUserRole").asArray(String.class);
                    Collection<SimpleGrantedAuthority> authorities = new ArrayList<>();
                    stream(appUserRole).forEach(role -> {
                        authorities.add(new SimpleGrantedAuthority("appUserRole"));
                    });
